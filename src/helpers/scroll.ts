...
const initScrollListeners = ({
  ...
  const dragScrollbar = (e: MouseEvent) => {
    ...
    if (isDragging) {
      const scrollContent = scrollWrap.querySelector(
        ".scrollContent",
      ) as HTMLElement;
      const contentElement = checkIsBodyParentComponent(scrollWrap)
        ? scrollWrap
        : scrollContent;
      ...
      if (direction === "vertical") {
        ...
        scrolledElement.scrollTo({
          top: contentTop + (contentHeight * delta) / 100,
        });
      } else {
        ...
        scrolledElement.scrollTo({
          left: contentLeft + (contentWidth * delta) / 100,
        });
      }
    }
  };
  ...
  scrollContainer.addEventListener("mousedown", (e: MouseEvent) => {
    ...
    startY = e.clientY;
    startX = e.clientX;
    contentTop = isBodyParentComponent
      ? window.scrollY
      : scrollContent.scrollTop;
    contentLeft = isBodyParentComponent
      ? window.scrollX
    ...
    if (e.target !== scrollIndicator) {
      ...
      if (direction === "vertical") {
        const contentHeight = isBodyParentComponent
        ...
        const contentWidth = isBodyParentComponent
...
const setHeight = (
  ...
  const scrollIndicator = scrollWrap.querySelector(
    ".scrollVertical",
  ) as HTMLElement;
  const scrollContent = scrollWrap.querySelector(
    ".scrollContent",
  ...
  if (scrollWrap && scrollIndicator && contentHeight && parentElement) {
    const containerHeight = checkIsBodyParentComponent(scrollWrap)
      ? parentElement.clientHeight
      : scrollWrap.scrollHeight;
    const scrollTop = isBodyParentComponent
      ? (scrolledElement as Window)?.scrollY
      : (scrolledElement as HTMLDivElement).scrollTop;
...
const setWidth = (
  ...
  if (scrollWrap && scrollIndicator && contentWidth && parentElement) {
    const containerWidth = checkIsBodyParentComponent(scrollWrap)
      ? parentElement.clientWidth
      : scrollWrap.scrollWidth;
    const scrollLeft = isBodyParentComponent
      ? (scrolledElement as Window).scrollX
      : (scrolledElement as HTMLDivElement).scrollLeft;
...
const checkIsBodyParentComponent = (scrollWrap: Element) =>
  scrollWrap.parentElement?.tagName === "BODY";

const scrollEndHandler = (scrollWrap: Element) => {
  if (!isDragging) {
    setTimeout(() => {
      const scrollVerticalIndicator = scrollWrap.querySelector(
        ".scrollVertical",
      ) as HTMLElement;
      const scrollHorizontalIndicator = scrollWrap.querySelector(
        ".scrollHorizontal",
      ) as HTMLElement;
...
const scrollEndPolyfill = (scrollWrap: Element) => {
  ...
  if (!isFirstScroll) {
    ...
    if (scrollVerticalIndicator) {
      ...
      if (scrollTop !== top) {
        ...
        scrollVerticalIndicator.style.boxShadow =
          "0 0 0 1px rgba(255, 255, 255, 0.35)";
        scrollVerticalIndicator.style.backgroundColor =
          "var(--color-scrollbar)";
      }
    }
    ...
    if (scrollHorizontalIndicator) {
      ...
      if (scrollLeft !== left) {
        ...
        scrollHorizontalIndicator.style.boxShadow =
          "0 0 0 1px rgba(255, 255, 255, 0.35)";
        scrollHorizontalIndicator.style.backgroundColor =
          "var(--color-scrollbar)";
      }
    }

    // polyfill for scrollend event for Safari
    // Mozilla Firefox also doesn't support the scrollend event correctly
    scrollEndPolyfill(scrollWrap);
  } else {
...
const resetBeforeUnload = (
  ...
  scrollWrap.removeEventListener(
    "resize",
    resizeHandler.bind(null, scrollWrap, scrolledElement),
  );
  scrollWrap.removeEventListener(
    "scroll",
    scrollHandler.bind(null, scrollWrap, scrolledElement),
  );
  scrollWrap.removeEventListener(
    "scrollend",
    scrollEndHandler.bind(null, scrollWrap),
  );
};
...
const initScroll = () => {
  ...
  scrollWraps.forEach((scrollWrap) => {
    scrollWrap.classList.add("transition", "relative");
    ...
    const scrolledElement = checkIsBodyParentComponent(scrollWrap)
      ? window
      : scrollContent;
    ...
    scrolledElement.addEventListener(
      "scrollend",
      scrollEndHandler.bind(null, scrollWrap),
    );
    ...
    window.addEventListener("beforeunload", () => {
      resetBeforeUnload.bind(null, scrollWrap, scrolledElement);
